...
def GetURFNodes(mol):
    ...
    ringlist = ring_info.AtomRingFamilies()
    return ringlist
...
def GetAromaticRings(mol):
    ...
    ringlist = ring_info.AtomRingFamilies()
    aromaticrings = [
        ring for ring in ringlist if all(mol.GetAtomWithIdx(atom).GetIsAromatic() for atom in ring)
    ]
...
dict_path = file_path.parent.absolute().joinpath("pipistackdict.txt")
with open(str(dict_path.absolute()), "r") as f:
    dictionary = f.read()
