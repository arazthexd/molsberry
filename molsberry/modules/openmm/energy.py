...
from .representations import OpenMMInputMolRep, OpenMMPreInputMolRep
...
class OpenMMEnergyCalculator(OpenMMInterface, SimpleBlock):
    ...
    inputs = [
        ("molecules", MoleculeData, OpenMMPreInputMolRep, False)
    ]
    outputs = [
    ...
    def operate(self, input_dict: Dict[str, OpenMMPreInputMolRep]) \
        ...
        for k in openmm_input_keys:
            input_dict[k].forcefield = self.forcefield

        result_dict = {
            "energy": self.calc_energy([input_dict[k].to_OpenMMInputMolRep() 
                                        for k in openmm_input_keys])
        }
    ...
    def calc_energy(self, 
                    omm_rep: OpenMMInputMolRep | List[OpenMMInputMolRep]) \
                        -> Dict[str, Any]:
